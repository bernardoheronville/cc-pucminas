...
Hash *criarHash() {
    ...
    nova->tam = 21;
    for(int i = 0; i < nova->tam; i++) nova->tabela[i] = criarLista();
    return nova;
}
...
int h(String x, Hash *hash) {
    int soma = 0;
    for(int i = 0; i < my_strlen(x.str); i++) soma += (int)x.str[i];
    return soma % hash->tam;
}

// Procedimento de inserir na hash.
...
// Procedimentos de pesquisa do hash.
bool pesquisarAux(String x, Hash *hash) {
    bool resp = false;
    int pos = h(x, hash);
    Lista *lista = hash->tabela[pos];
    Celula *i;
    for (i = lista->primeiro->prox; i != NULL; i = i->prox) {
        if (my_strcmp(i->elemento.nome.str, x.str)) { 
            resp = true;
            break;
        }
    }
